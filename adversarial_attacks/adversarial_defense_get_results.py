...
parser.add_argument("--index_dataset",type=int)
...
excel_dir = father_directory + "trained_model_results/excel_result/{}/{}/".format(training_method_name, model_name)
print(model_name)
print(dataset_name)
...
def cw_attack_l2(model, imgs, labels):
    freeze(model)
    imgs = imgs.to(device)
    labels = labels.to(device)
    inputs_box = (min((0 - m) / s for m, s in zip(mean, std)), 
                  max((1 - m) / s for m, s in zip(mean, std)))
    l2_attack = cw.L2Adversary(targeted=False,
                            confidence=0.0,
                            search_steps=5,
                            max_steps=100,
                            box=inputs_box,
                            optimizer_lr=5e-4)
    adv_imgs = l2_attack(model, imgs, labels, to_numpy = False).to(device)
    return adv_imgs

def bim(model, X, y, criterion, epsilon, alpha = 1/255, alpha2 = 0):
    freeze(model)
    num_of_iteration = int(min(4 + epsilon / alpha, 1.25 * epsilon / alpha))
    ...
    pre_sign = torch.zeros_like(X)

    for ite in range(num_of_iteration):
        nX = nX.clone().detach()
        ...
        loss.backward()
        grad_x = nX.grad
        nnX = nX + (torch.ones_like(nX)+alpha2*(grad_x.detach().sign()==pre_sign))*alpha_normed*grad_x.detach().sign()
        pre_sign = grad_x.detach().sign()    
        nX = torch.clamp(nnX, 0, 1)
    return nX
...
def adver_method(model, images, labels, method):
    if(method == 'BIM'): 
        return bim(model, images, labels, bim_criterion, bim_epsilon, alpha = bim_alpha)
    if(method == 'BIM2'):
        return bim(model, images, labels, bim_criterion, bim_epsilon, alpha = bim_alpha, alpha2 = 1)
    if(method == 'BIM3'):
        return bim(model, images, labels, bim_criterion, bim_epsilon, alpha = bim_alpha, alpha2 = -0.5)
    if(method == 'FGSM'): 
    ...
    if(method == 'CW_L2'):
        return cw_attack_l2(model, images, labels)
...
if __name__=='__main__':
    ...
    try:
        ...
        excel_path = excel_dir+"top_1_top_5.xlsx"
        book =  initWorkbook()
